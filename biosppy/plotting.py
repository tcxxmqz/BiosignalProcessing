...
def plot_eda_qz(ts=None,
             raw=None,
             filtered=None,
             onsets=None,
             offsets=None,
             peaks=None,
             eda_phasic_diffandsmoothed=None,
             path=None,
             show=False):
    """Create a summary plot from the output of signals.eda.eda.
    ...
    # ax2.vlines(ts[offsets], ymin, ymax,
    #            color='r',
    #            linewidth=MINOR_LW,
    #            label='Offsets')
    ...
    ax2.scatter(ts[onsets], filtered[onsets], marker='o', label='SCR-Onsets')
    ax2.scatter(ts[peaks], filtered[peaks], marker='^', label='SCR-Peaks')
    ax2.scatter(ts[offsets], filtered[offsets], marker='x', label='SCR-Offsets')

    # SCR反应区域标记上颜色
    i = 0
    for i in range(0, len(ts[onsets]) - 1):
        ax2.axvspan(xmin=ts[onsets][i], xmax=ts[offsets][i], facecolor='gray', alpha=0.3)
    ax2.axvspan(xmin=ts[onsets][-1], xmax=ts[offsets][-1], facecolor='gray', alpha=0.3, label='SCR')
    ...
    # eda_phasic_diffandsmoothed
    ax3 = fig.add_subplot(313, sharex=ax1)
    ax3.plot(ts[:len(eda_phasic_diffandsmoothed)], eda_phasic_diffandsmoothed,label='EDA diff&smoothed')

    # ax3.hlines(0, 0, ts[-1], colors='r', linestyles='dashed')
    ax3.axhline(color='orange', ls='dashed')

    ax3.scatter(ts[onsets], eda_phasic_diffandsmoothed[onsets], marker='o', label='SCR-Onsets')
    ax3.scatter(ts[peaks], eda_phasic_diffandsmoothed[peaks], marker='^', label='SCR-Peaks')
    ax3.scatter(ts[offsets], eda_phasic_diffandsmoothed[offsets], marker='x', label='SCR-Offsets')

    # SCR反应区域标记上颜色
    i = 0
    for i in range(0, len(ts[onsets]) - 1):
        ax3.axvspan(xmin=ts[onsets][i], xmax=ts[offsets][i], facecolor='gray', alpha=0.3)
    ax3.axvspan(xmin=ts[onsets][-1], xmax=ts[offsets][-1], facecolor='gray', alpha=0.3, label='SCR')
