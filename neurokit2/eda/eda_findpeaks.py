...
def eda_findpeaks(eda_phasic, sampling_rate=1000, method="neurokit", amplitude_min=0.1):
    ...
    elif method in ["nabian2018qz", "nabianqz", "qz", "qizhen"]:
        info = _eda_findpeaks_nabian2018qz(eda_phasic)
    else:
        raise ValueError(
...
def _eda_findpeaks_nabian2018qz(eda_phasic):
    """Basic method to extract Skin Conductivity Responses (SCR) from an EDA signal following the
    approach by Nabian et al. (2018).
    ...
    onsets : array
        Indices of the SCR onsets.
    offsets : array
        Indices of the SCR offsets. # modified by qz
    peaks : array
        Indices of the SRC peaks.
    amplitudes : array
        SCR pulse amplitudes.
    ...
    Modified
    ----------
     The occurrence of a SCR is detected by findingtwoconsecutivezero-crossings,fromnegativetopos-itive 
     and positive to negative of the bartletted differentiated EDAsignal.

     We considered negative to positive as the beginning and positive to negative as the end of each SCR.
     原版函数没有考虑清晰正负过零点问题，应考虑到保证配对后的SCR开始与结束索引值的大小：pos_crossings > neg_crossings
     同时在返回值中添加SCR_Offsets
     modified by qizhen in 2020-10-23 9:56
    ...
    # smooth
    eda_phasic = signal_smooth(df, kernel="bartlett", size=20)
    eda_phasic_diffandsmoothed = eda_phasic
    ...
    for i, j in zip(pos_crossings, neg_crossings):
        ...
        if not diff < (0.1 * amp):
            ...
            onsets_list.append(i)
            amps_list.append(amp)
            offsets_list.append(j)  # modified by qz

    # output
    # info = {"SCR_Onsets": np.array(onsets_list), "SCR_Peaks": np.hstack(np.array(peaks_list)),
    #         "SCR_Height": np.array(amps_list)}

    info = {"SCR_Onsets": np.array(onsets_list), "SCR_Offsets": np.array(offsets_list), "SCR_Peaks": np.hstack(np.array(peaks_list)),
            "SCR_Height": np.array(amps_list), "eda_phasic_diffandsmoothed": eda_phasic_diffandsmoothed}  # modified by qz
